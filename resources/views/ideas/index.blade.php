<x-layout title="Ideas | Explore Your Ideas">
    <div>
        ...
        <div class="my-5 flex justify-start items-center gap-4">
            <a class="btn {{ !request('status') ? 'btn-pirmary pointer-events-none' : '' }}" href="/ideas">All <span
                    class="text-sm text-white ps-3">{{ $statusCount->get('all') }}</span></a>
            @foreach (App\IdeaStatus::cases() as $status)
                <a class="btn {{ request('status') === $status->value ? ' btn-primary pointer-events-none' : '' }}"
                    href="/ideas?status={{ $status->value }}">{{ $status->value }} <span
                        class="text-sm text-white ps-3">{{ $statusCount->get($status->value) }}</span></a>
            @endforeach
